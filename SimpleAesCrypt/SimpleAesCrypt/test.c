...
static int test_decrypt_cbc_with_param(uint8_t *key, uint8_t *in)
{
    uint8_t iv[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };

    struct AES_ctx ctx;
    int key_len = strlen(in);

    AES_init_ctx_iv(&ctx, key, iv);
    AES_CBC_encrypt_buffer(&ctx, in, key_len);
    ...
    AES_init_ctx_iv(&ctx, key, iv);
    AES_CBC_decrypt_buffer(&ctx, in, key_len);

    printf("\n");
    printf("CBC decrypted: %s", in);
}

int main(void)
{
    // Declare variables for input strings
    char inputString1[MAX_LENGTH] = { 0 };
    char inputString2[MAX_LENGTH] = { 0 };
    ...
    inputString2[strcspn(inputString2, "\n")] = '\0';

    // Convert input strings to arrays of bytes
    unsigned char bytes1[MAX_LENGTH] = { 0 };
    unsigned char bytes2[MAX_LENGTH] = { 0 };

    // Copy characters from input strings to byte arrays
    strcpy((char*)bytes1, inputString1);
    strcpy((char*)bytes2, inputString2);

    // Display bytes in a loop for the first string
    // printf("\nBytes for the first string:\n");
    // for (int i = 0; i < strlen(inputString1); i++) {
    //     printf("%02X ", bytes1[i]);
    // }
    // printf("\n");
    // 
    // // Display bytes in a loop for the second string
    // printf("\nBytes for the second string:\n");
    // for (int i = 0; i < strlen(inputString2); i++) {
    //     printf("%02X ", bytes2[i]);
    // }
    // printf("\n");

    test_decrypt_cbc_with_param(bytes2, bytes1);
